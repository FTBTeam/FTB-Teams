...
### Added
* Added `ClientManager#openTeamPropertiesEditor` API method
  * allow other mods to open the team properties editor (e.g. FTB Chunks map)

### Fixed
* Fixed team screen color button not updating immediately on team color change
* `/ftbteams {party|server} settings ...` commands now correctly treat a value of `""` or `''` as an empty string

## [26.1.2.3-beta]

### Added
* Added `TeamPropertiesChangedEvent#hasPropertyChanged` convenience method

### Fixed
* Fixed boolean team properties not deserializing correctly

## [26.1.2.2-beta]

### Changed
* See `26.1.2.1-beta` for previous changes
