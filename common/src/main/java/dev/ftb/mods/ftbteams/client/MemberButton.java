		...
		if (player.uuid.equals(self.uuid)) {
			if (selfRank.is(TeamRank.OWNER)) {
				if (selfTeam.getMembers().size() == 1) {
					items0.add(new ContextMenuItem(new TranslatableComponent("ftbteams.gui.disband"), Icons.CLOSE,
							() -> Operation.LEAVE.sendMessage(player))
							.setYesNo(new TranslatableComponent("ftbteams.gui.disband.confirm")));
				}
			} else {
				items0.add(new ContextMenuItem(new TranslatableComponent("ftbteams.gui.leave"), Icons.CLOSE,
						() -> Operation.LEAVE.sendMessage(player))
						.setYesNo(new TranslatableComponent("ftbteams.gui.leave.confirm")));
			}
		} else {
			if (selfRank.is(TeamRank.OWNER)) {
				if (playerRank == TeamRank.MEMBER) {
					items0.add(new ContextMenuItem(new TranslatableComponent("ftbteams.gui.promote", player.name), Icons.SHIELD,
							() -> Operation.PROMOTE.sendMessage(player))
							.setYesNo(new TranslatableComponent("ftbteams.gui.promote.confirm", player.name)));
				} else if (playerRank == TeamRank.OFFICER) {
					items0.add(new ContextMenuItem(new TranslatableComponent("ftbteams.gui.demote", player.name), Icons.ACCEPT_GRAY,
							() -> Operation.DEMOTE.sendMessage(player))
							.setYesNo(new TranslatableComponent("ftbteams.gui.demote.confirm", player.name)));
				}
				if (playerRank.isMember()) {
					items0.add(new ContextMenuItem(new TranslatableComponent("ftbteams.gui.transfer_ownership", player.name), Icons.DIAMOND,
							() -> Operation.TRANSFER_OWNER.sendMessage(player))
							.setYesNo(new TranslatableComponent("ftbteams.gui.transfer_ownership.confirm", player.name)));
				}
			}
		}
		if (selfRank.getPower() > playerRank.getPower()) {
			if (playerRank.isMember()) {
				items0.add(new ContextMenuItem(new TranslatableComponent("ftbteams.gui.kick", player.name), Icons.CLOSE,
						() -> Operation.KICK.sendMessage(player))
						.setYesNo(new TranslatableComponent("ftbteams.gui.kick.confirm", player.name)));
			} else if (selfRank.isOfficer() && playerRank.isAlly()) {
				items0.add(new ContextMenuItem(new TranslatableComponent("ftbteams.gui.remove_ally", player.name), Icons.CANCEL,
						() -> Operation.REMOVE_ALLY.sendMessage(player))
						.setYesNo(new TranslatableComponent("ftbteams.gui.remove_ally.confirm", player.name)));
				}
		}

		if (!items0.isEmpty()) {
			List<ContextMenuItem> items = new ArrayList<>(ImmutableList.of(
