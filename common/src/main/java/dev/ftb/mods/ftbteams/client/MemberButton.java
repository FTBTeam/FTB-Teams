...
public class MemberButton extends NordButton {
	...
	public void drawIcon(PoseStack matrixStack, Theme theme, int x, int y, int w, int h) {
		...
		if (ClientTeamManager.INSTANCE.selfTeam().getType() == TeamType.PARTY) {
			TeamRank tr = ClientTeamManager.INSTANCE.selfTeam().getHighestRank(player.id());
	...
	public void onClicked(MouseButton button) {
		if (ClientTeamManager.INSTANCE.self() == null || ClientTeamManager.INSTANCE.selfTeam() == null) return;

		KnownClientPlayer self = ClientTeamManager.INSTANCE.self();
		ClientTeam selfTeam = ClientTeamManager.INSTANCE.selfTeam();
		TeamRank selfRank = selfTeam.getHighestRank(self.id());
		TeamRank playerRank = selfTeam.getHighestRank(player.id());
