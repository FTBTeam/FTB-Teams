		...
		dispatcher.register(Commands.literal("ftbteams")
				.then(Commands.literal("party")
						...
						.then(Commands.literal("leave")
								...
								.executes(ctx -> getPartyTeam(ctx, TeamRank.MEMBER).leave(ctx.getSource().getPlayerOrException()))
						)
						...
						.then(Commands.literal("kick")
								.requires(source -> hasParty(source, TeamRank.OFFICER))
								.then(Commands.argument("players", GameProfileArgument.gameProfile())
										.executes(ctx -> getPartyTeam(ctx, TeamRank.OFFICER).kick(ctx.getSource().getPlayerOrException(), GameProfileArgument.getGameProfiles(ctx, "players")))
								)
						)
						.then(Commands.literal("transfer_ownership")
								.requires(source -> hasParty(source, TeamRank.OWNER))
								.then(Commands.argument("player", EntityArgument.player())
										.executes(ctx -> getPartyTeam(ctx, TeamRank.OWNER).transferOwnership(ctx.getSource().getPlayerOrException(), EntityArgument.getPlayer(ctx, "player")))
								)
						)
